...
var getSorter = ({
  ...
  if (caseSensitive && natural) {
    return (a, b) => (0, import_natural_compare.default)(a, b);
  } else if (caseSensitive) {
    return (a, b) => a < b ? -1 : a > b ? 1 : 0;
  } else if (natural) {
    return (a, b) => (0, import_natural_compare.default)(a.toLowerCase(), b.toLowerCase());
  }
...
// src/resolver.ts
var import_isomorphic_resolve = __toESM(require("isomorphic-resolve"), 1);
...
function isResolved(source) {
  ...
  try {
    (0, import_isomorphic_resolve.default)(source);
...
var import_experimental_utils2 = require("@typescript-eslint/experimental-utils");

// src/ts-utils.ts
var import_experimental_utils = require("@typescript-eslint/experimental-utils");
function getName2(node) {
  switch (node?.type) {
    case import_experimental_utils.AST_NODE_TYPES.Identifier:
      return node.name;
    case import_experimental_utils.AST_NODE_TYPES.Literal:
      return node.value.toString();
    case import_experimental_utils.AST_NODE_TYPES.TemplateLiteral:
      return node.quasis.reduce(
...
function getWeight(node) {
  const weights = {
    [import_experimental_utils2.AST_NODE_TYPES.TSConstructSignatureDeclaration]: 0,
    [import_experimental_utils2.AST_NODE_TYPES.TSCallSignatureDeclaration]: 1,
    [import_experimental_utils2.AST_NODE_TYPES.TSPropertySignature]: 2,
    [import_experimental_utils2.AST_NODE_TYPES.TSMethodSignature]: 2,
    [import_experimental_utils2.AST_NODE_TYPES.TSIndexSignature]: 3
  };
  return weights[node.type];
}
function getSortValue3(node) {
  switch (node.type) {
    case import_experimental_utils2.AST_NODE_TYPES.TSPropertySignature:
    case import_experimental_utils2.AST_NODE_TYPES.TSMethodSignature:
      return getName2(node.key);
    case import_experimental_utils2.AST_NODE_TYPES.TSIndexSignature:
      return getName2(node.parameters[0]);
  }
  return "";
}
var type_properties_default = import_experimental_utils2.ESLintUtils.RuleCreator.withoutDocs({
  create(context) {
    const source = context.getSourceCode();
    const options = context.options[0];
    const sorter = getSorter(options);
...
var import_experimental_utils3 = require("@typescript-eslint/experimental-utils");
function getSortValue4(node) {
  return node.type === import_experimental_utils3.TSESTree.AST_NODE_TYPES.TSLiteralType && node.literal.type === import_experimental_utils3.TSESTree.AST_NODE_TYPES.Literal && typeof node.literal.value === "string" ? node.literal.value : null;
}
var string_unions_default = import_experimental_utils3.ESLintUtils.RuleCreator.withoutDocs({
  create(context) {
    const source = context.getSourceCode();
    const options = context.options[0];
    const sorter = getSorter(options);
...
var import_experimental_utils4 = require("@typescript-eslint/experimental-utils");
function getSortValue5(node) {
  return node.initializer?.type === import_experimental_utils4.TSESTree.AST_NODE_TYPES.Literal && typeof node.initializer.value === "string" ? node.initializer.value : null;
}
var string_enums_default = import_experimental_utils4.ESLintUtils.RuleCreator.withoutDocs({
  create(context) {
    const source = context.getSourceCode();
    const options = context.options[0];
    const sorter = getSorter(options);
